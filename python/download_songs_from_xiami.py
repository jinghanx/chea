...
for index in range(1, 100):
    if 'song_%d.xml' % index not in os.listdir('songs_xml'):
        os.system('curl http://data.xiami.com/widget/xml-single/uid/0/sid/%d > songs_xml/song_%d.xml' % (index, index))
    try:
        tree = ET.parse('songs_xml/song_%d.xml' % index)
        root = tree.getroot()
        ...
        if artist_name.encode('utf-8') not in os.listdir('songs'):
            os.system(('mkdir songs/%s' % artist_name).encode('utf-8'))
        if album_name.encode('utf-8') not in os.listdir(('songs/%s' % artist_name).encode('utf-8')):
            os.system(('mkdir songs/%s/%s' % (artist_name, album_name)).encode('utf-8'))
        #response = urllib.urlopen(location_decoded)
        response = br.open(location_decoded)
        html = response.read()
        song_file = open(('songs/%s/%s/%s.mp3' % (artist_name, album_name, song_name)).encode('utf-8'), 'w')
        song_file.write(html)
        #os.system(('curl %s > songs/%s/%s/%s.mp3' % (location_decoded, artist_name, album_name, song_name)).encode('utf-8'))
    except:
        continue
